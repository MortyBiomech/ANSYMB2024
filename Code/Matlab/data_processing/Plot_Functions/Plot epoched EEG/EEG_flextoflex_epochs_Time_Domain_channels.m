function EEG_flextoflex_epochs_Time_Domain_channels(data, std_sem)
    ...
    %%% making 3d matrix 
    numChannels = size(data.EEG_P1_flextoflex.Time_Domain.channels.length_normalized{1,1,1} , 1);
    
    numPointsP1   = size(data.EEG_P1_flextoflex.Time_Domain.channels.length_normalized{1,1,1} , 2);
    numEpochsP1   = size(data.EEG_P1_flextoflex.Time_Domain.channels.length_normalized , 3);
    signal_P1_3d = zeros(numChannels, numPointsP1, numEpochsP1);
    countP1 = 0;
    for i = 1:numEpochsP1
        tempdata = data.EEG_P1_flextoflex.Time_Domain.channels.length_normalized{1,1, i};
        if ~isempty(tempdata)
    ...
    numPointsP3   = size(data.EEG_P3_flextoflex.Time_Domain.channels.length_normalized{1,1,1} , 2);
    numEpochsP3   = size(data.EEG_P3_flextoflex.Time_Domain.channels.length_normalized , 3);
    signal_P3_3d = zeros(numChannels, numPointsP3, numEpochsP3);
    countP3 = 0;
    for i = 1:numEpochsP3
        tempdata = data.EEG_P3_flextoflex.Time_Domain.channels.length_normalized{1,1, i};
        if ~isempty(tempdata)
    ...
    numPointsP6   = size(data.EEG_P6_flextoflex.Time_Domain.channels.length_normalized{1,1,1} , 2);
    numEpochsP6   = size(data.EEG_P6_flextoflex.Time_Domain.channels.length_normalized , 3);
    signal_P6_3d = zeros(numChannels, numPointsP6, numEpochsP6);
    countP6 = 0;
    for i = 1:numEpochsP6
        tempdata = data.EEG_P6_flextoflex.Time_Domain.channels.length_normalized{1,1, i};
        if ~isempty(tempdata)
    ...
    tiledlayout(8,8)
    sgtitle(sprintf('Time-Domain (Flex-to-Flex, Mean $\\pm$ %s): %d epochs P1, %d epochs P3, %d epochs P6', ...
