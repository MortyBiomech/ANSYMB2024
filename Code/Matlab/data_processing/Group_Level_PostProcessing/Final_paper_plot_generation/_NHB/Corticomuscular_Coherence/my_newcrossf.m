...
%
%   Based on NEWCROSSF (Arnaud Delorme, Sigurd Enghoff & Scott Makeig,
%   CNL/Salk Institute 1998-2001; SCCN/INC/UCSD, 2002-)
%
% %% MK MODIFICATION SUMMARY (Morteza Khosrotabar, TU Darmstadt, 2025):
%
%   Three targeted changes from original NEWCROSSF:
%
%   (1) Added 'timewarp', 'timewarpms', 'timewarpidx' to accepted params.
%       Added ms-to-frames conversion block (same logic as NEWTIMEF).
%
%   (2) TF decomposition runs WITHOUT timestretch on both X and Y.
%       Per-trial cross-spectrum (crossspec) and per-trial power
%       (alltfX_pow, alltfY_pow) are computed first.
%
%   (3) Timewarping is applied AFTER coherence computation:
%       - crossspec    [freq x time x trials] warped trial by trial
%         (amplitude via timewarp, phase via angtimewarp)
%       - alltfX_pow   [freq x time x trials] warped trial by trial
%         (amplitude only via timewarp)
%       - alltfY_pow   [freq x time x trials] warped trial by trial
%         (amplitude only via timewarp)
%       - coherres     [freq x time] computed from warped quantities
%
%   This approach is more correct than warping X and Y independently
%   because the cross-spectrum captures the joint EEG-EMG relationship
%   and warping it preserves the phase difference between signals.
%
%   All modifications marked with %% MK MODIFICATION comments.
%
% Usage:
...
%   'timewarp'   = [matrix] (nTrials x nEvents) matrix of per-trial event
%                  latencies in ms. From EEG.timewarp.latencies.
%                  Pass only the good epochs (after epochs_to_keep filter).
%   'timewarpms' = [vector] (1 x nEvents) target latencies in ms to warp
%                  events to. From EEG.timewarp.warpto (group median).
%                  If empty, median of 'timewarp' columns is used.
%   'timewarpidx'= [vector] indices of events to mark with vertical lines.
%
% All other optional inputs identical to NEWCROSSF.
%
% Outputs:
%   coherres          = [freq x time] warped linear coherence (MSC)
...
%   crossspec_warped  = [freq x time x trials] warped per-trial cross-spectrum
%   alltfX            = [freq x time x trials] EEG TF decomposition (unwarped)
%   alltfX_pow_warped = [freq x time x trials] warped EEG power
%   alltfY            = [freq x time x trials] EMG TF decomposition (unwarped)
...
%       crossspec_warped, alltfX, alltfX_pow_warped,               ...
%       alltfY, alltfY_pow_warped]                                 ...
%         = my_newcrossf(x, y, 2000, [-500 3500], 500, [3 0.8],   ...
...
function [coherres, mbase, timesout, freqs, Rbootout, Rangle,    ...
          ...
          alltfY, alltfY_pow_warped]                              ...
    = my_newcrossf(X, Y, frame, tlimits, Fs, varwin, varargin)
...
elseif (~isnumeric(frame) || length(frame)~=1 || frame~=round(frame))
    fprintf('my_newcrossf(): frames must be an integer.\n'); return
elseif (frame <= 0)
    fprintf('my_newcrossf(): frames must be positive.\n'); return
elseif ~iscell(X) && (rem(size(X,2),frame)~=0) && (rem(size(X,1),frame)~=0)
    fprintf('my_newcrossf(): data length must be divisible by frames.\n'); return
end
...
elseif (tlimits(1) >= tlimits(2))
    error('my_newcrossf(): tlimits must be [min max].');
end
...
try, g.type;          catch, g.type          = 'coher';         end  % MK: default changed to coher
...
%% MK MODIFICATION (1): defaults for timewarp parameters
...
for index = 1:length(allfields)
    switch allfields{index}
        ...
        case {'plotersp' 'plotitc'}
            disp(['my_newcrossf warning: timef option ''' allfields{index} ''' ignored']);
        otherwise
            disp(['my_newcrossf error: unrecognized option ''' allfields{index} '''']);
            beep; return;
    end
end
...
if strcmpi(g.title, DEFAULT_TITLE)
    switch g.type
        case 'coher',       g.title = 'Event-Related Coherence (MSC)';
        case 'phasecoher',  g.title = 'Event-Related Phase Coherence';
        case 'phasecoher2', g.title = 'Event-Related Phase Coherence 2';
        case 'amp',         g.title = 'Amplitude Correlation';
        case 'crossspec',   g.title = 'Cross-Spectrum';
    end
end
...
if g.tlimits(2)-g.tlimits(1) < 30
    disp('my_newcrossf WARNING: time range very small (<30ms). Check ms/s units.');
end

% =========================================================================
%% MK MODIFICATION (1 cont.): Convert timewarp ms -> frames -> timeStretch
%  Identical logic to newtimef.m
% =========================================================================
if ~isempty(g.timewarp)
    fprintf('\nmy_newcrossf: Timewarp parameters detected.\n');
    fprintf('  Warping will be applied to cross-spectrum after TF decomposition.\n');
    ...
    if ~isempty(g.timewarpms)
        refms  = g.timewarpms;   % [1 x nEvents] in ms
    ...
    if max(g.timeStretchMarks(:)) > frame-2 || min(g.timeStretchMarks(:)) < 3
        error('my_newcrossf: Warp event latencies must be inside the epoch.');
    end
    if ~isempty(g.timeStretchRefs)
        if max(g.timeStretchRefs(:)) > frame-2 || min(g.timeStretchRefs(:)) < 3
            error('my_newcrossf: Warp reference latencies must be inside the epoch.');
        end
    end
    ...
    fprintf('\nmy_newcrossf: No timewarp provided — running without warping.\n');
end
%% END MK MODIFICATION (1 cont.)

% =========================================================================
% Shuffle trials / detrend (identical to newcrossf)
% =========================================================================
if g.shuffle ~= 0
...
if ~isnan(g.alpha)
    fprintf('Bootstrap significance: alpha = %g\n', g.alpha);
else
    fprintf('No bootstrap significance testing.\n');
end
...
% =========================================================================
%% MK MODIFICATION (2): TF decomposition WITHOUT timestretch
%  Warping is applied separately to cross-spectrum and power below.
% =========================================================================
fprintf('\nComputing TF decomposition of EEG (no warping at this stage)...\n');
X = reshape(X, g.frame, g.trials);
[alltfX, freqs, timesout] = timefreq(X, g.srate, spectraloptions{:});

fprintf('\nComputing TF decomposition of EMG (no warping at this stage)...\n');
Y = reshape(Y, g.frame, g.trials);
[alltfY] = timefreq(Y, g.srate, spectraloptions{:});
%% END MK MODIFICATION (2)

% =========================================================================
%% MK MODIFICATION (3): Compute per-trial cross-spectrum and power,
%  then timewarp all three quantities trial by trial.
% =========================================================================

% --- Step A: per-trial cross-spectrum and power (unwarped) ---
fprintf('\nComputing per-trial cross-spectrum and power...\n');
crossspec_noWarp  = alltfX .* conj(alltfY);   % [freq x time x trials]
alltfX_pow_noWarp = abs(alltfX).^2;           % [freq x time x trials]
alltfY_pow_noWarp = abs(alltfY).^2;           % [freq x time x trials]

% --- Step B: timewarp trial by trial if timewarp was supplied ---
if ~isempty(g.timeStretchMarks)

    fprintf('\nApplying timewarp to cross-spectrum and power trial by trial...\n');

    % reconstruct TF time grid indices from timesout (ms -> frames)
    % this mirrors what timefreq does internally with g.indexout
    ...
    [~, refsPos] = min(abs( ...
        repmat(timerefs,    [1 length(indexout)]) - ...
        repmat(indexout, [length(timerefs) 1])       ...
        )');
    refsPos(end+1) = 1;
    refsPos(end+1) = length(indexout);
    refsPos = sort(refsPos);

    % preallocate warped outputs
    crossspec_warped  = zeros(size(crossspec_noWarp));
    alltfX_pow_warped = zeros(size(alltfX_pow_noWarp));
    alltfY_pow_warped = zeros(size(alltfY_pow_noWarp));
    ...
    for t = 1:g.trials
        ...
        [~, marksPos] = min(abs( ...
            repmat(timemarks(:,t), [1 length(indexout)]) - ...
            repmat(indexout, [size(timemarks,1) 1])          ...
            )');
        marksPos(end+1) = 1;
        marksPos(end+1) = length(indexout);
        marksPos = sort(marksPos);

        % build warp matrix once per trial (shared across freq)
        M = timewarp(marksPos, refsPos);   % [nTFtimes x nTFtimes]

        % --- warp cross-spectrum ---
        % decompose into amplitude and phase
        cs     = crossspec_noWarp(:,:,t);   % [freq x time]
        cs_r   = abs(cs);                   % amplitude
        cs_phi = angle(cs);                 % phase [-pi, pi]

        % warp amplitude (linear interp via matrix multiply)
        TSr = transpose(M * cs_r');         % [freq x nTargetTimes]

        % warp phase (circular interp, per frequency)
        TSphi = zeros(size(cs_phi));
        for fi = 1:size(cs_phi, 1)
            TSphi(fi,:) = angtimewarp(marksPos, refsPos, cs_phi(fi,:));
        end

        % recombine complex cross-spectrum
        crossspec_warped(:,:,t) = TSr .* exp(1i * TSphi);

        % --- warp EEG power (amplitude only — no phase) ---
        px = alltfX_pow_noWarp(:,:,t);      % [freq x time] real
        alltfX_pow_warped(:,:,t) = transpose(M * px');

        % --- warp EMG power (amplitude only — no phase) ---
        py = alltfY_pow_noWarp(:,:,t);      % [freq x time] real
        alltfY_pow_warped(:,:,t) = transpose(M * py');

    end
    fprintf('Warping complete.\n');

else
    % no warping — warped = unwarped
    crossspec_warped  = crossspec_noWarp;
    alltfX_pow_warped = alltfX_pow_noWarp;
    alltfY_pow_warped = alltfY_pow_noWarp;
    fprintf('No warping applied — warped outputs equal unwarped.\n');
end

% --- Step C: compute coherres from warped quantities ---
fprintf('\nComputing coherence from warped cross-spectrum and power...\n');
switch g.type
    case 'coher'
        % Linear coherence (MSC):
        % numerator   = sum of warped cross-spectrum across trials
        % denominator = sqrt(sum_X_pow * sum_Y_pow)
        num   = sum(crossspec_warped, 3);                        % [freq x time]
        denX  = sum(alltfX_pow_warped, 3);                       % [freq x time]
        denY  = sum(alltfY_pow_warped, 3);                       % [freq x time]
        coherres = num ./ sqrt(denX .* denY);                    % [freq x time]

    case 'phasecoher'
        % Phase coherence (PLV):
        % normalize each trial's cross-spectrum to unit amplitude first
        cs_norm  = crossspec_warped ./ abs(crossspec_warped);    % [freq x time x trials]
        coherres = sum(cs_norm, 3) / g.trials;                   % [freq x time]

    case 'phasecoher2'
        coherres = sum(crossspec_warped, 3) ./ ...
                   sum(abs(crossspec_warped), 3);                 % [freq x time]

    case 'crossspec'
        coherres = crossspec_warped;                             % [freq x time x trials]

    case 'amp'
        alltfX_amp = sqrt(alltfX_pow_noWarp);
        alltfY_amp = sqrt(alltfY_pow_noWarp);
...
% =========================================================================
% Bootstrap (identical to newcrossf — skipped for crossspec/amp types)
% =========================================================================
if ~isempty(g.rboot)
    Rbootout = g.rboot;
elseif ~isnan(g.alpha) && ~strcmpi(g.type,'crossspec') && ~strcmpi(g.type,'amp')
    % use warped cross-spectrum for bootstrap
    switch g.type
        case 'coher'
            inputdata = {crossspec_warped, alltfX_pow_warped, alltfY_pow_warped};
            formula   = ['sum(arg1,3) ./ sqrt(sum(arg2,3).*sum(arg3,3));'];
        case 'phasecoher'
            cs_norm   = crossspec_warped ./ abs(crossspec_warped);
            inputdata = {cs_norm};
            formula   = 'mean(arg1,3);';
        case 'phasecoher2'
            inputdata = {crossspec_warped};
...
fprintf('\nmy_newcrossf complete.\n');
fprintf('  coherres          [%s] — warped linear coherence\n', ...
    num2str(size(coherres)));
fprintf('  crossspec_warped  [%s] — warped per-trial cross-spectrum\n', ...
    num2str(size(crossspec_warped)));
fprintf('  alltfX            [%s] — EEG TF decomposition (unwarped)\n', ...
    num2str(size(alltfX)));
fprintf('  alltfX_pow_warped [%s] — warped EEG power\n', ...
    num2str(size(alltfX_pow_warped)));
fprintf('  alltfY            [%s] — EMG TF decomposition (unwarped)\n', ...
    num2str(size(alltfY)));
fprintf('  alltfY_pow_warped [%s] — warped EMG power\n', ...
    num2str(size(alltfY_pow_warped)));

return;

% =========================================================================
% SUBFUNCTIONS (identical to newcrossf)
% =========================================================================
